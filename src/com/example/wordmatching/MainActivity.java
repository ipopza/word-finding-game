...
public class MainActivity extends SimpleBaseGameActivity {
	...
	protected Scene onCreateScene() {
		...
		Sprite sprite = new Sprite(0, 0, backgroundTextureRegion, getVertexBufferObjectManager());
		...
		for (y = 0; y < dimenY; y++) {
			...
			for (x = 0; x < dimenX; x++) {
				...
				spriteTable[x][y] = new WordSprite(x, y, posX, posY, textureRegionList.get(table[x][y]), getVertexBufferObjectManager());
		...
		for (Word w : wordList) {
			w.text = new Text(posX, posY, this.mFont, w.value.toUpperCase(), new TextOptions(HorizontalAlign.LEFT), getVertexBufferObjectManager());
		...
		timerText = new Text(posX, posY, this.mFont, getTime(), new TextOptions(HorizontalAlign.LEFT), getVertexBufferObjectManager());
